...
const PaymentMethodForm = ({
  ...
  return (
    ...
    <Form {...form}> <form method='post' onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
        ...
        <div className='flex flex-col gap-5 md:flex-row'>
           {/*  formfield start */}
            <FormField control={form.control} name='type' render={({ field }) => (
                ...
                <FormMessage />  {/*  the message for validation */}

                </FormItem>
            )}

            />
             {/*  formfield finished with '/>' */}

        </div>

        <div className='flex gap-2'>

            <Button type='submit' disabled={isPending}> {/* Loading effect plays inside the button */}
            {isPending ? (
